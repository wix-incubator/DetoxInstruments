...
+ (void)_redirectLogOutput
{
	...
	__log_queue = dispatch_queue_create("com.wix.DTXProfilerLogIOQueue", dispatch_queue_attr_make_with_qos_class(DISPATCH_QUEUE_SERIAL, QOS_CLASS_USER_INTERACTIVE, 0));
...
{
	pthread_mutex_lock(&__recordingProfilersMutex);
	[__recordingProfilers addObject:listener];
	pthread_mutex_unlock(&__recordingProfilersMutex);
}
...
{
	pthread_mutex_lock(&__recordingProfilersMutex);
	[__recordingProfilers removeObject:listener];
	pthread_mutex_unlock(&__recordingProfilersMutex);
}
